<template>
  <div class="tasks_box p-1 p-sm-2 p-md-3 p-lg-4 p-xl-5 bg-white shadow rounded h-100">
        <h4 class="border-start border-4 border-danger ps-2 my-1">TASKS ELIMINATE</h4>
        <div @click="clearTrash()" class="my_btn btn-light w-100 border mb-2"> Svuota Lista </div>

        <div v-if="trash_tasks == '' " class="alert alert-info text-center ">Il cestino è vuoto  🛒</div>
 
        <div v-else class="task my-1 ps-2 d-flex justify-content-between align-items-center" v-for="(task, index) in trash_tasks" :key="task.id"> 
            <span>{{task}} </span>
...
export default {
    ...
    methods: {
        restoreTask(task, index){
            ...
            this.saveTasks();
            this.trash_tasks.splice(index, 1);
            this.saveTrash();
        },
        clearTrash(){
            this.trash_tasks = [];
